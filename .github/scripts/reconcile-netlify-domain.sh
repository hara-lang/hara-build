...
curl --fail --silent --show-error \
  ...
  --data "$(jq -nc --argjson aliases "$aliases" '{domain_aliases: $aliases, force_ssl: true}')" \
  "$api" >/dev/null
...
if [[ "$current" != "$NETLIFY_CUSTOM_DOMAIN" ]]; then
  ...
  for attempt in 1 2 3 4; do
    status="$({ curl --silent --show-error \
      ...
      -H "$auth" \
      -H 'Content-Type: application/json' \
      --data "$(jq -nc --arg domain "$NETLIFY_CUSTOM_DOMAIN" '{custom_domain: $domain, force_ssl: true}')" \
      "$api"; } || true)"
    if [[ "$status" =~ ^2 ]]; then
      break
    fi
...
echo "Netlify now routes ${NETLIFY_CUSTOM_DOMAIN} with ${NETLIFY_LEGACY_DOMAIN} as a redirect alias."
